...
function build_table(result){
	...
	$.each(orderlist,function(index,item){
		...
		var orderstate=$("<td></td>").append(orderstate1).attr("align","center");
		var orderprice=$("<td></td>").append(item.orderprice).attr("align","center");
		var searchod=$("<td></td>").append($("<a></a>").append("详细信息").addClass("od").attr("od-id",item.oid).attr("style","color:blue")).attr("align","center");
		var delBtn = $("<td></td>").append($("<a></a>").append("收货").addClass("del").attr("del-id",item.oid).attr("style","color:blue")).attr("align","center");
...
function build_page_nav(result){
	...
	if(result.obj.pageInfo.hasPreviousPage == false){
		...
		prePageLi.click(function(){
			getMyOrderList(result.obj.pageInfo.pageNum -1);
		});
	}
	...
	if(result.obj.pageInfo.hasNextPage == false){
		...
		lastPageLi.click(function(){
			getMyOrderList(result.obj.pageInfo.pages);
		});
	}
	//添加首页和前一页 的提示
...
$(document).on("click",".del",function(){
	orders_handle($(this).attr("del-id"),2);
});
$(document).on("click",".od",function(){
	orders_info($(this).attr("od-id"));
});
